...
!function(){angular.module("marcuraUI.components").directive("maSelectBox",["$timeout","maHelper",function(e,t){return{restrict:"E",scope:{items:"=",selectedItem:"=",isLoading:"=",change:"&",itemTemplate:"=",itemTextField:"@",itemValueField:"@",isDisabled:"=",isRequired:"=",isSearchable:"=",canAddItem:"=",addItemTooltip:"="},replace:!0,template:function(){var e='            <div class="ma-select-box"                ng-class="{                    \'ma-select-box-can-add-item\': canAddItem,                    \'ma-select-box-is-focused\': isFocused                }">                <div class="ma-select-box-spinner" ng-if="isLoading">                    <div class="pace">                        <div class="pace-activity"></div>                    </div>                </div>                <select ui-select2="options"                    ng-show="!addingItem"                    ng-disabled="isDisabled"                    ng-model="value"                    ng-change="onChange()"                    ng-required="isRequired">                    <option ng-repeat="item in items" value="{{getItemValue(item)}}">                        {{formatItem(item)}}                    </option>                </select>                <input class="ma-select-box-input" type="text" ng-show="addingItem"                    ng-model="text"                    ng-focus="onFocus()"                    ng-blur="onBlur()"/>                <ma-button ng-if="canAddItem" size="xs" modifier="secondary"                    tooltip="{{getAddItemTooltip()}}"                    right-icon="{{addingItem ? \'bars\' : \'plus\'}}"                    click="addItem()">                </ma-button>            </div>';return e},controller:["$scope",function(e){e.options={},e.isSearchable||(e.options.minimumResultsForSearch=-1)}],link:function(i,l){var n,d,s=angular.element(l[0].querySelector(".ma-select-box-input"));i.addingItem=!1,i.formatItem=i.itemTemplate||function(e){return e?i.itemTextField?e[i.itemTextField]:e.toString():""},i.isFocused=!1,i.selectedItem&&(i.itemValueField?i.value=i.selectedItem[i.itemValueField].toString():"string"==typeof i.selectedItem?i.value=i.selectedItem:i.value=JSON.stringify(i.selectedItem),n=i.value),i.getAddItemTooltip=function(){return i.addingItem?"Back to the list":i.addItemTooltip?i.addItemTooltip:"Add new item"},i.getItemValue=function(e){return i.itemValueField?e[i.itemValueField].toString():e},i.addItem=function(){i.addingItem=!i.addingItem,i.addingItem?(n=i.selectedItem,i.selectedItem=d):(d=i.selectedItem,i.selectedItem=n),e(function(){if(i.change({item:i.selectedItem}),i.addingItem)s.focus(),i.isFocused=!0;else{var e=angular.element(l[0].querySelector(".select2-container"));e.select2("focus")}})},i.onFocus=function(){i.isFocused=!0},i.onBlur=function(){if(i.isFocused=!1,i.itemTextField){if(i.selectedItem&&i.selectedItem[i.itemTextField]===i.text)return;i.selectedItem={},i.selectedItem[i.itemTextField]=i.text}else{if(i.selectedItem===i.text)return;i.selectedItem=i.text}d=i.selectedItem,e(function(){i.change({item:i.selectedItem})})},i.onChange=function(){var l=t.isJson(i.value)?JSON.parse(i.value):i.value;if(i.itemValueField)for(var d=0;d<i.items.length;d++)if(i.items[d][i.itemValueField].toString()===l.toString()){l=i.items[d];break}if(l||i.selectedItem){if(i.itemValueField){if(i.selectedItem&&i.selectedItem[i.itemValueField].toString()===l[i.itemValueField].toString())return}else if(l===i.selectedItem)return;i.selectedItem=l,n=l,e(function(){i.change({item:l})})}}}}}])}();
