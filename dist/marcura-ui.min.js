...
!function(){angular.module("marcuraUI.components").filter("maSelectBoxOrderBy",["orderByFilter",function(e){return function(t,i){return i?e(t,i):t}}]).directive("maSelectBox",["$document","$timeout","maHelper","maValidators",function(e,t,i,l){return{restrict:"E",scope:{id:"@",items:"=",selectedItem:"=",isLoading:"=",change:"&",blur:"&",focus:"&",itemTemplate:"=",itemTextField:"@",itemValueField:"@",isDisabled:"=",isRequired:"=",validators:"=",canSearch:"=",canAddItem:"=",addItemTooltip:"@",showAddItemTooltip:"=",instance:"=",orderBy:"=",ajax:"=",canReset:"=",placeholder:"@",textPlaceholder:"@"},replace:!0,template:function(e,t){var l=!i.isNullOrWhiteSpace(t.ajax),s="                    <div class=\"ma-select-box\"                        ng-class=\"{                            'ma-select-box-can-add-item': canAddItem,                            'ma-select-box-is-text-focused': isTextFocused,                            'ma-select-box-is-disabled': isDisabled,                            'ma-select-box-is-invalid': !isValid,                            'ma-select-box-is-touched': isTouched,                            'ma-select-box-mode-add': isAddMode,                            'ma-select-box-mode-select': !isAddMode,                            'ma-select-box-can-reset': canReset,                            'ma-select-box-is-reset-disabled': canReset && !isDisabled && !isResetEnabled()                        }\">                        <div class=\"ma-select-box-spinner\" ng-if=\"isLoading && !isDisabled\">                            <div class=\"pace\">                                <div class=\"pace-activity\"></div>                            </div>                        </div>";return s+=l?'<input ui-select2="options"                        ng-show="!isAddMode"                        ng-disabled="isDisabled"                        ng-change="onChange()"                        ng-model="value"                        placeholder="{{placeholder}}"/>':'                        <select ui-select2="options"                            ng-show="!isAddMode"                            ng-disabled="isDisabled"                            ng-model="value"                            ng-change="onChange()"                            placeholder="{{placeholder}}">                            <option></option>                            <option ng-repeat="item in items | maSelectBoxOrderBy:orderBy" value="{{getItemValue(item)}}">                                {{formatItem(item)}}                            </option>                        </select>',s+='                    <input class="ma-select-box-text" type="text" ng-show="isAddMode"                        ng-model="text"                        ng-disabled="isDisabled"                        ng-focus="onFocus(\'text\')"                        placeholder="{{textPlaceholder}}"/>                    <ma-button class="ma-button-switch"                        ng-if="canAddItem" size="xs" modifier="simple"                        tooltip="{{getAddItemTooltip()}}"                        right-icon="{{isAddMode ? \'bars\' : \'plus\'}}"                        click="toggleMode()"                        ng-focus="onFocus()"                        is-disabled="isDisabled">                    </ma-button>                    <ma-button class="ma-button-reset"                        ng-show="canReset" size="xs" modifier="simple"                        right-icon="times-circle"                        click="onReset()"                        ng-focus="onFocus()"                        is-disabled="!isResetEnabled()">                    </ma-button>                </div>'},controller:["$scope",function(e){e.options={},e.canSearch||(e.options.minimumResultsForSearch=-1),e.ajax&&(e.options.ajax=e.ajax,e.options.minimumInputLength=3,e.options.escapeMarkup=function(e){return e},e.options.initSelection=function t(i,l){if(t.runs=t.runs?t.runs:1,1===t.runs&&e.selectedItem&&e.selectedItem[e.itemValueField]){var s=angular.copy(e.selectedItem);s.text=e.itemTemplate?e.itemTemplate(s):s[e.itemTextField],s.id=s[e.itemValueField],e.previousSelectedItem=s,l(s)}else l();t.runs++})}],link:function(s,o){var n,d,c,a,u,r=angular.element(o[0].querySelector(".ma-select-box-text")),m=null,f=!0,g=!1,p=s.showAddItemTooltip!==!1,x=s.validators?angular.copy(s.validators):[],I=s.isRequired,h=!1;s.previousSelectedItem=s.previousSelectedItem||null,s.isAddMode=!1,s.formatItem=s.itemTemplate||function(e){return e?s.itemTextField?e[s.itemTextField]:e.toString():""},s.isTextFocused=!1,s.isValid=!0,s.isTouched=!1,s.isAjax=angular.isObject(s.ajax);var b=function(e){if(!angular.isArray(s.items))return!1;for(var t=s.itemValueField&&e[s.itemValueField],i=0;i<s.items.length;i++)if(t){if(s.items[i][s.itemValueField]===e[s.itemValueField])return!0}else if(s.items[i]===e)return!0;return!1},v=function(e){if(!e)return null;if(!s.itemTextField)return e;if(angular.isArray(s.items))for(var t=0;t<s.items.length;t++)if(s.items[t][s.itemValueField].toString()===e.toString())return s.items[t];return null},F=function(e){if(!s.itemTextField)return e;if(e){var t={};return t[s.itemTextField]=e,t}return null},A=function(e){s.canAddItem&&e&&(s.isAddMode=!b(e)),S(e),s.isAddMode?(e?s.itemTextField&&e[s.itemTextField]?s.text=e[s.itemTextField].toString():s.text=e:s.text=null,m=e,s.toggleMode("add")):(e?s.isAjax||(s.itemValueField&&e[s.itemValueField]?s.value=e[s.itemValueField].toString():"string"==typeof e&&(s.value=e)):s.value=null,s.previousSelectedItem=e,s.toggleMode("select"))},T=function(e,i){var l=angular.element(e.relatedTarget),o=angular.element(a.dropdown[0].querySelector(".select2-input"));s.isTextFocused=!1,"text"===i?(g=!1,s.$apply(function(){if(s.isTouched=!0,s.itemTextField){if(s.selectedItem&&s.selectedItem[s.itemTextField]===s.text)return;s.selectedItem=F(s.text)}else{if(s.selectedItem===s.text)return;s.selectedItem=s.text}m=s.selectedItem,s.isValid&&t(function(){s.change({item:s.selectedItem})})})):"select"===i&&s.$apply(function(){s.isTouched=!0}),f=s.canAddItem?!g&&l[0]!==n[0]&&l[0]!==d[0]&&l[0]!==r[0]&&l[0]!==a.focusser[0]&&l[0]!==o[0]:!g&&l[0]!==d[0]&&l[0]!==r[0]&&l[0]!==a.focusser[0]&&l[0]!==o[0],f&&s.blur({item:s.selectedItem}),g=!1},S=function(e){if(s.isValid=!0,x&&x.length)for(var t=0;t<x.length;t++)if(!x[t].method(e)){s.isValid=!1;break}},M=function(){s.isAddMode?(r.focus(),s.isTextFocused=!0):c.select2("focus")};s.isResetEnabled=function(){return!s.isDisabled&&(s.isAddMode?!i.isNullOrWhiteSpace(s.text):!i.isNullOrUndefined(s.selectedItem))},s.onReset=function(){s.selectedItem=null,M(),t(function(){s.change({item:s.selectedItem})})},s.onFocus=function(e){"text"===e&&(s.isTextFocused=!0),f&&s.focus({item:s.selectedItem}),f=!1},r.focusout(function(e){T(e,"text")}),s.getAddItemTooltip=function(){return p?s.isAddMode?"Back to the list":s.addItemTooltip?s.addItemTooltip:"Add new item":""},s.getItemValue=function(e){return s.itemValueField?e[s.itemValueField].toString():e},s.toggleMode=function(e){if(s.canAddItem&&!(s.isAddMode&&"add"===e||!s.isAddMode&&"select"===e)){var i=!1;"select"===e?(s.isAddMode=!1,i=!0):"add"===e?(s.isAddMode=!0,i=!0):s.isAddMode=!s.isAddMode,s.isAddMode?(c&&c.select2("close"),s.previousSelectedItem=v(s.value),s.selectedItem=m,s.selectedItem&&(s.text="string"==typeof s.selectedItem?s.selectedItem:s.selectedItem[s.itemTextField])):(m=F(s.text),s.selectedItem=s.previousSelectedItem),i||t(function(){s.change({item:s.selectedItem}),M()})}},s.onChange=function(){var e=i.isJson(s.value)?JSON.parse(s.value):s.value;if(s.isAjax){if(e===s.previousSelectedItem)return;if(!angular.isObject(e))return}if(!s.isAjax&&s.itemValueField&&!i.isNullOrWhiteSpace(e))for(var l=0;l<s.items.length;l++)if(s.items[l][s.itemValueField].toString()===e.toString()){e=s.items[l];break}if(e||s.selectedItem){if(s.itemValueField){if(s.selectedItem&&s.selectedItem[s.itemValueField]&&s.selectedItem[s.itemValueField].toString()===e[s.itemValueField].toString())return}else if(e===s.selectedItem)return;s.selectedItem=e,s.previousSelectedItem=e,t(function(){s.change({item:e})})}},s.$watch("selectedItem",function(e,t){e!==t&&A(e)}),s.$watch("text",function(e,t){e!==t&&(s.isTouched=!0,S(e))}),s.instance&&(s.instance.switchToSelectMode=function(){s.isAddMode&&s.toggleMode("select")},s.instance.switchToAddMode=function(){s.isAddMode||s.toggleMode("add")},s.instance.isValid=function(){return s.isValid});for(var V=0;V<x.length;V++)if("IsNotEmpty"===x[V].name){h=!0;break}!h&&I&&x.unshift(l.isNotEmpty()),h&&(I=!0),t(function(){A(s.selectedItem),c=angular.element(o[0].querySelector(".select2-container")),a=c.data().select2,u=$('label[for="'+s.id+'"]'),n=angular.element(o[0].querySelector(".ma-button-switch")),d=angular.element(o[0].querySelector(".ma-button-reset")),u.length>0&&$(e).on("click",'label[for="'+s.id+'"]',function(){M()}),a.focusser.on("focus",function(){s.onFocus("select")}),a.focusser.on("focusout",function(e){T(e,"select")}),a.dropdown.on("focus",".select2-input",function(){g=!0,s.onFocus()}),a.dropdown.on("focusout",".select2-input",function(e){T(e,"select")}),n.focusout(function(e){T(e)}),d.focusout(function(e){T(e)}),a.dropdown.on("mouseenter",".select2-result",function(){g=!0}),$(e).on("mouseenter",".select2-drop-mask",function(){g=!0})})}}}])}();
